...
# Save aggregated multivariate fault code data set so we don't have to rerun
write.csv(fully_joined, "final_data-v3.csv", row.names=FALSE)
```

#Read in aggregated multivariate fault code data set
fully_joined <- read.csv("Data/final_data-v3.csv")
View(fully_joined)

#Convert timestamp from char to date time 
fully_joined$Timestamp <- ymd_hms(fully_joined$Timestamp)
str(fully_joined)

#Delete all duplicate rows
fully_joined <- fully_joined[!duplicated(fully_joined[,4:5]),]
View(fully_joined)

#Order based on Turbine and date
fully_joined <- fully_joined %>% arrange(Turbine, Timestamp)
View(fully_joined)
...
path_out = 'Data\\'
write.csv(workorder,paste(path_out,'fuzzy_workorder.csv',sep = ''),row.names=FALSE)
workorder <- read.csv("Data/fuzzy_workorder.csv")
# convert dates from numeric to date format
workorder$wo_start_date <- ymd(workorder$wo_start_date)
workorder$wo_start_date <- format(workorder$wo_start_date,"%Y-%m-%d %H:%M:%S")

workorder$wo_finish_date <- ymd(workorder$wo_finish_date)





workorder$round_date <- lubridate::round_date(workorder$wo_start_date, "15 minutes")
...
path_out = 'Data\\'
write.csv(fully_joined,paste(path_out,'final_data-v3.csv',sep = ''),row.names=FALSE)
...
data = pd.read_csv("Data/final_data-v3.csv")
