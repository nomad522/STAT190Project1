...
path_out = 'Data/'
write.csv(fully_joined,paste(path_out,'final_data-v7.csv',sep = ''),row.names=FALSE)

# load save point
fully_joined <- read.csv("Data/final_data-v7.csv")
...
# remove windspeed
ts_imputed = subset(ts_imputed, select =-c(mean_windspeed, max_windspeed, min_windspeed, sd_windspeed))
# list of turbines
...
column_list = colnames(ts_imputed)[c(3:34)]
...
for (i in 1:length(turbine_list)){
 ...
 x = subset(fully_joined, location_id == turbine_list[i])
...
write.csv(ts_imputed,paste(path_out,'final_data-v7-imputed.csv',sep = ''),row.names=FALSE)
...
#Read in raw dataset
db <- read.csv("Data/final_data-v7-imputed.csv", stringsAsFactors = TRUE)
...
db2 = db %>% group_by(location_id) %>% mutate(lag_mean_bearing = lag(mean_bearing, n=24),
                                          lag_max_bearing = lag(max_bearing, n=24),
                                          lag_min_bearing = lag(min_bearing, n=24),
                                          lag_sd_bearing = lag(sd_bearing, n=24),
                                          lag_mean_hydraulic = lag(mean_hydraulic, n=24),
                                          lag_max_hydraulic = lag(max_hydraulic, n=24),
                                          lag_min_hydraulic = lag(min_hydraulic, n=24),
                                          lag_sd_hydraulic = lag(sd_hydraulic, n=24),
                                          lag_mean_rpm = lag(mean_rpm, n=24),
                                          lag_max_rpm = lag(max_rpm, n=24),
                                          lag_min_rpm = lag(min_rpm, n=24),
                                          lag_sd_rpm = lag(sd_rpm, n=24),
                                          lag_mean_oil = lag(mean_oil, n=24),
                                          lag_max_oil = lag(max_oil, n=24),
                                          lag_min_oil = lag(min_oil, n=24),
                                          lag_sd_oil = lag(sd_oil, n=24),
                                          lag_mean_ims = lag(mean_ims, n=24),
                                          lag_max_ims = lag(max_ims, n=24),
                                          lag_min_ims = lag(min_ims, n=24),
                                          lag_sd_ims = lag(sd_ims, n=24),
                                          lag_mean_ims2 = lag(mean_ims2, n=24),
                                          lag_max_ims2 = lag(max_ims2, n=24),
                                          lag_min_ims2 = lag(min_ims2, n=24),
                                          lag_sd_ims2 = lag(sd_ims2, n=24),
                                          lag_mean_active = lag(mean_ims, n=24),
                                          lag_max_active = lag(max_active, n=24),
                                          lag_min_active = lag(min_active, n=24),
                                          lag_sd_active = lag(sd_active, n=24),
                                          lag_mean_ambient = lag(mean_ambient, n=24),
                                          lag_max_ambient = lag(max_ambient, n=24),
                                          lag_min_ambient = lag(min_ambient, n=24),
                                          lag_sd_ambient = lag(sd_ambient, n=24))
                                          

db2 = db2 %>% drop_na(lag_min_hydraulic)
db2 = subset(db2, select=-c(max_bearing,min_bearing,sd_bearing,mean_hydraulic,max_hydraulic,min_hydraulic,sd_hydraulic,mean_rpm,max_rpm,min_rpm,sd_rpm,mean_oil,max_oil,min_oil,sd_oil,mean_ims,max_ims,min_ims,sd_ims,mean_ims2,max_ims2,min_ims2,sd_ims2,mean_ims,max_active,min_active,sd_active,mean_ambient,max_ambient,min_ambient,sd_ambient))
# Output Dataset imputed lagged dataset
write.csv(db2,paste(path_out,'final_data-v7-imputed-lagged.csv',sep = ''),row.names=FALSE)

lagged <- read.csv("Data/final_data-v7-imputed-lagged.csv", stringsAsFactors = TRUE)
# after reading in, must convert time date again.
lagged$Timestamp <- ymd_hms(lagged$Timestamp)
...
```{r, random_forest}
#WIP
# find most effective m

forest_data <- lagged
forest_data = forest_data %>% mutate(BinaryFault = case_when(FaultCodeType =="Critical" ~ 1,
                                               .default = 0))

forest_data$BinaryFault <- as.factor(forest_data$BinaryFault)
forest_data$Turbine <- as.factor(forest_data$Turbine)
