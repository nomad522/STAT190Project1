...
for (i in 2:length(wo_list)){
  new = read.csv(paste0("Data/Wind Turbine Data Batch 2/Work Order/", wo_list[i]), header = TRUE)
  wo = rbind(wo, new)
}
...
# keeping only one observation of each fault code of each turbine's 15 minute interval
...
bearing <- bearing %>% group_by(location_id, round_date) %>% mutate(mean_bearing = mean(Temperature))
# keeping only one observation of the mean for each turbine's 15 minute interval
bearing <- bearing %>% distinct(location_id, round_date, .keep_all = TRUE)
# keeping only a subset of columns
bearing = subset(bearing, select = c(1,6,7))
...
ims <- ims %>% group_by(location_id, round_date) %>% mutate(mean_ims= mean(Temperature))
# # Removing keeping only one observation of the mean for each turbine's 15 minute interval
ims <- ims %>% distinct(location_id, round_date, .keep_all = TRUE)
# Note: Tried a different method in getting the mean and deleting duplicates. Results in the same number of observations.
# keeping only a subset of columns
ims = subset(ims, select = c(1,6,7))
...
#adding mean ims2 bearing temp of the 15 minute intervals and dropping columns
ims2 <- ims2 %>% group_by(location_id, round_date) %>% mutate(mean_ims2 = mean(Temperature))
# # Removing keeping only one observation of the mean for each turbine's 15 minute interval
ims2 <- ims2 %>% distinct(location_id, round_date, .keep_all = TRUE)
# keeping only a subset of columns
ims2 = subset(ims2, select = c(1,6,7))
...
oil <- oil %>% group_by(location_id, round_date) %>% mutate(mean_oil = mean(Temperature))
# # Removing keeping only one observation of the mean for each turbine's 15 minute interval
oil <- oil %>% distinct(location_id, round_date, .keep_all = TRUE)
# keeping only a subset of columns
oil = subset(oil, select = c(1,6,7))
...
rpm <- rpm %>% group_by(location_id, round_date) %>% mutate(mean_rpm = mean(RPM))
# # Removing keeping only one observation of the mean for each turbine's 15 minute interval
rpm <- rpm %>% distinct(location_id, round_date, .keep_all = TRUE)
# keeping only a subset of columns
rpm = subset(rpm, select = c(1,6,7))
...
#adding hydraulic pressure to 15 minute intervals and dropping columns
hydraulic <- hydraulic %>% group_by(location_id, round_date) %>% mutate(mean_oil = mean(Pressure))
# Removing keeping only one observation of the mean for each turbine's 15 minute interval
hydraulic <- hydraulic %>% distinct(location_id, round_date, .keep_all = TRUE)
# keeping only a subset of columns
hydraulic = subset(hydraulic, select = c(1,6,7))
...
# adding mean active power of the 15 minute intervals and dropping columns
active <- active %>% group_by(location_id, round_date) %>% mutate(mean_power = mean(power))
# # Removing keeping only one observation of the mean for each turbine's 15 minute interval
active <- active %>% distinct(location_id, round_date, .keep_all = TRUE)
# keeping only a subset of columns
active = subset(active, select = c(1,6,7))
...
path_out = 'Data/'
write.csv(fully_joined,paste(path_out,'final_data-v6.csv',sep = ''),row.names=FALSE)

# load save point
fully_joined <- read.csv("Data/final_data-v6.csv")
...
column_list = colnames(ts_imputed)[c(7:15)]
...
write.csv(ts_imputed,paste(path_out,'Data/final_data-v6-imputed.csv',sep = ''),row.names=FALSE)

# Everything looks good except windspeed imputes to over 600 m/s = over 1000mph...
ts_imputed %>% ungroup()%>% summarise(min = min(Windspeed),
                        max = max(Windspeed),)
ts_imputed %>% ungroup()%>% summarise(min = min(Bearing_Temp),
                        max = max(Bearing_Temp),)
ts_imputed %>% ungroup()%>% summarise(min = min(Generator_RPM),
                        max = max(Generator_RPM),)
ts_imputed %>% ungroup()%>% summarise(min = min(Hydraulic_Pressure),
                        max = max(Hydraulic_Pressure),)
ts_imputed %>% ungroup()%>% summarise(min = min(Oil_Temp),
                        max = max(Oil_Temp),)
ts_imputed %>% ungroup()%>% summarise(min = min(Gearbox_IMS_1),
                        max = max(Gearbox_IMS_1),)
ts_imputed %>% ungroup()%>% summarise(min = min(Gearbox_IMS_2),
                        max = max(Gearbox_IMS_2),)
ts_imputed %>% ungroup()%>% summarise(min = min(Active_Power),
                        max = max(Active_Power),)
ts_imputed %>% ungroup()%>% summarise(min = min(Ambient_Temp),
                        max = max(Ambient_Temp),)
ts_imputed %>% ungroup()%>% summarise(min = min(Timestamp),
                        max = max(Timestamp),)
...
#Read in raw dataset
db <- read.csv("Data/final_data-v6-imputed.csv", stringsAsFactors = TRUE)
...
data = pd.read_csv("Data/final_data-v6-imputed.csv")
...
db2 = db %>% group_by(Turbine) %>% mutate(lagHSB = lag(Bearing_Temp, n=24),
                                           lagPressure = lag(Hydraulic_Pressure, n=24),
                                           lagRPM = lag(Generator_RPM, n=24),
                                           lagOil = lag(Oil_Temp, n=24),
                                           lagIMS1 = lag(Gearbox_IMS_1, n=24),
                                           lagIMS2 = lag(Gearbox_IMS_2, n=24),
                                           lagPower = lag(Active_Power, n=24))
db2 = db2 %>% drop_na(lagHSB)
db2 = subset(db2, select=-c(Windspeed,Ambient_Temp, Bearing_Temp,Hydraulic_Pressure,Generator_RPM,Oil_Temp,Gearbox_IMS_1,Gearbox_IMS_2,Active_Power))
write.csv(db2, "lagged_final.csv", row.names=FALSE)

lagged <- read.csv("Data/lagged_final.csv", stringsAsFactors = TRUE)
# after reading in, must convert time date again.
lagged$Timestamp <- ymd_hms(db$Timestamp)
...
```{r, random_forest}
#WIP
# find most effective m try

final_data2 <- lagged
str(final_data2)

final_data2 = final_data2 %>% mutate(BinaryFault = case_when(FaultCodeType =="Critical" ~ 1,
                                               .default = 0))

final_data2$BinaryFault <- as.factor(final_data2$BinaryFault)
final_data2$location_id <- as.factor(final_data2$Turbine)
str(final_data2)

RNGkind(sample.kind = "default")
set.seed(231231)
train.idx <- sample(x = 1 : nrow(final_data2), size = .8*nrow(final_data2))
# make training data
train.df <- final_data2[train.idx, ]
#make testing data
test.df <- final_data2[-train.idx, ]


mtry <- c(1:8) # The maximum number of x variables are 8
#make room for m, OOB error
keeps2 <- data.frame(m = rep(NA,length(mtry)*10),#NOTE DIFFERENCE
                     OOB_err_rate = rep(NA, length(mtry)*10))#NOTE DIFFERENCE
j = 0 #initialize row to fill#NOTE DIFFERENCE
for (rep in 1:10){#NOTE DIFFERENCE
  ...
  for (idx in 1:length(mtry)){
    ...
    keeps2[j ,"OOB_err_rate"] <- mean(predict(tempforest)!= train.df$Outcome)#NOTE DIFFERENCE
  }
}
keeps2
#calculate mean for each m value
keeps3 <- keeps2 %>% group_by(m) %>% summarise(mean_oob = mean(OOB_err_rate))
keeps3

#need to update with m with smallest oob error
finalForest <- randomForest(BinaryFault ~ Turbine + lagHSB + lagPressure + lagRPM + lagOil + lagIMS1 + lagIMS2 + lagPower, # dangers here
             data = train.df,
             ntree = 1000, # b
             mtry = 2,
             importance = TRUE)
