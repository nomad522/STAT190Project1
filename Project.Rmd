...
for (i in 1:nrow(fully_joined)){
  if (fully_joined$Discription[i] == "Gear Oil Temperature High"||
      ...
      fully_joined$Discription[i] == "Rpm Sensor Error"|| ## Genrpm/Srsg Speed error mentioned in Daniel's email?
      fully_joined$Discription[i] == "Srsg Activated"|| ##only instance with srsg so i included it 
      fully_joined$Discription[i] == "Lms-Rot Gearbearing Temp Too High" # Not actually in our fault code descriptions but was in the email from Daniel
  ...
  else {fully_joined$FaultCodeType[i] = "Unknown"}
})
```
