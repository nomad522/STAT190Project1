...
# uses the unimputed data set. This data set has both the ageragted data (by the mean, max, min, and standard deviation) for 1 
# hour intervals and that data lagged by 6 hours
...
mtry <- c(1:10) # The maximum number of independent variables are 32. sqrt(32) ~ 5.6, but we tried 10
...
keeps
# plotting the number of variables in the model against the oob. The best oob seems to be 3
ggplot(data = keeps)+
...
# Sensitivity = .538
# AUC = .743
# This means the model will predict a critical fault code 53.8% of the time correctly when there is actually a critical issue and
# that the model will predict non critical occurrences 94.0% of the time correctly when there is actually non critical occurrences.
