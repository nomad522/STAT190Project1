...
```
All r code and rmarkdown will be in the root directory with all sensory data will are to be in the directory Data/ and extracted into their respective sub directories as stated:\
...
# sorting data by turbine, then the DateTime.
active <- active %>% arrange(Turbine, DateTime) %>% distinct()
...
```{r matrix, echo=FALSE,warning=FALSE,message=FALSE,error=FALSE, results='hide',fig.keep='all'}
...
## Multivariate
```{r multivariate, figures-side, fig.show="hold", out.width="50%", fig.align="center"}
...
ggplot(db,
       aes(x=mean_oil_temp,
           ...
           group=FaultOccurred, 
           color=FaultOccurred))+
  ggtitle("Mean Active Power Vs Mean Gearbox IMS Bearing Temperature")+
