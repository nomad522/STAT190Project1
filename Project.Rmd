...
db <- read.csv("Data/final_data-v2.csv")
...
db <- subset(db, select = -c(DateTime, Date, FaultCode, StatusCode, Description, Type))
...
db <- db[!duplicated(db[,8:9]),]
#Order based on Turbine and date
names(db) <- c("Turbine", "Timestamp", "Bearing_Temp", "Windspeed", "Hydraulic_Pressure", "Generator_RPM", "Oil_Temp", "Gearbox_IMS", "Active_Power", "Fault_Occurred")
...
# 1. Show code
# 2. Show graphs
#   Research questions: What is the optimal temperature of components to prevent fault and maximize energy produced?
#                       Which variables are related to our temperature vcariables?
# 3. Descriptive: Use a logistic regression model with log link for binary Y variable
# X Variables: all numeric sensor reading data, categorical X variables?
# Y Variable: Binary Fault_Occurred Y variable
# Predictive: K-Means clustering due to large data size, k = 3?

# Issues Encountered
# 1. Runtime issues due to size of datasets
# 2. Visualizations can be misleading
# 3. How to join the work order data to the multivariate data set?

# Plans for next week
# 1. Create better visualizations to assess variable relationships
# 2. Begin selecting variables for models and creating regression models
